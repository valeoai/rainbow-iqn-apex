...
def sample_in_thread(mp_queue_sample, mem_redis, batchsize, queue_size):
    while(True):
        ...
        for sample_factor_iter in range(sample_factor):
            ...
            mp_queue_sample.put([tree_idxs[sample_factor_iter*step_tree_idxs:(sample_factor_iter+1)*step_tree_idxs],
                                 tab_byte_transition[sample_factor_iter*step_tab_byte_transition:(sample_factor_iter+1)*step_tab_byte_transition],
                                 weights[sample_factor_iter*step_weights:(sample_factor_iter+1)*step_weights]])
...
if args.continue_experiment:
    ...
    capacity_before_learning = args.memory_capacity # We fill the memory before learning when restarting an experiment (for a more fair restart...)
else:
...
mem_learner.priority_weight = mem_learner.priority_weight + (T - args.learn_start) * priority_weight_increase
...
mp_queue_sample = Queue(args.queue_size) #Queue filled by mem_learner.sample() and read to make the backprop...
mp_queue_update_priorities = Queue(args.queue_size) #Queue filled with priorities of batch and read to update them in the redis_database
...
while T < args.T_max:
    ...
    # Train and test
    if memory_got_enough_experience or mem_learner.transitions.get_current_capacity() >= capacity_before_learning:
        if not memory_got_enough_experience:
            ...
            p = Process(target=sample_in_thread, args=(mp_queue_sample, mem_learner, args.batch_size, args.queue_size))
            p.daemon = True
            p.start()
        ...
        if T % args.replay_frequency == 0:
            idxs, loss = learner.learn(mem_learner, mp_queue_sample)  # Train with n-step distributional double-Q learning
            ...
            if len(buffer_loss) == args.queue_size:
                ...
                for batch_priorities_iter in range(args.queue_size):
                    mp_queue_update_priorities.put([tab_idxs[batch_priorities_iter*args.batch_size:(batch_priorities_iter+1)*args.batch_size],
                                                    tab_priorities_cpu[batch_priorities_iter*args.batch_size:(batch_priorities_iter+1)*args.batch_size]])
        ...
        time_to_wait = 10
        print("not enough experience in memory, we wait " + str(time_to_wait) + " seconds before trying again")
